...
const Tablenew = (prop) => {
  ...
  const checkIfFilled = (index) => {
    console.log("This is tableCols and tableData: ", tableCols.length, Object.keys(tableData[0]).length)
    for (const prop in tableData[index]) {
      if (tableData[index][prop]) {
        console.log(tableData[index][prop])
        // console.log(index, tableData[index], tableData[index][prop])
        continue;
      } else {
        if (Object.keys(tableData[index]).length > tableCols.length) {
          Object.keys(tableData[index]).length = 0;
          continue;
        }
        // const {key} = tableData[index][prop]
        // console.log(key)
        // if(key == undefined) return true;
        console.log("stuck in loop!", tableData[index][prop])
        return false;
      }
    }
    return true;
  ...
  const addRow = () => {
    const key = (tableData.length == 0) ? 0 : tableData[tableData.length - 1].key + 1;
    ...
    if (tableData.length === 0) {
      ...
      const fillStatus = checkIfFilled(tableData.length == 0 ? 0 : tableData.length - 1);
      if (fillStatus) {
        setNewRow([0, 0]);
